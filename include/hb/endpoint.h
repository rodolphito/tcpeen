...
typedef struct hb_endpoint_s {
	...
	hb_sockaddr_storage_t sockaddr;
	uint8_t padding[HB_ENDPOINT_MAX_SIZE - sizeof(uint64_t) - sizeof(hb_sockaddr_storage_t)];
} hb_endpoint_t;
...
int hb_endpoint_convert_from(hb_endpoint_t *endpoint, void *sockaddr);

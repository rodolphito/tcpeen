...
HB_TEST_CASE_BEGIN(test_endpoint)
	...
	ASSERT_SUCCESS(hb_endpoint_convert_from(&endpoint, &sa));
	...
	ASSERT_SUCCESS(hb_endpoint_convert_from(&endpoint, &sa6));
	ASSERT_SUCCESS(hb_endpoint_get_string(&endpoint, buf, 255));
	ASSERT_SUCCESS(strcmp(buf, "1123::4567:890a:bcde:8045"));
	
	// test set ipv4
	ASSERT_SUCCESS(hb_endpoint_set_ip4(&endpoint, "0.0.0.0", 65432));
	ASSERT_SUCCESS(hb_endpoint_get_string(&endpoint, buf, 255));
	ASSERT_SUCCESS(strcmp(buf, "0.0.0.0:65432"));

	// test set ipv6
	ASSERT_SUCCESS(hb_endpoint_set_ip6(&endpoint, "fe80::2c92:d74a:43ba", 12345));
	ASSERT_SUCCESS(hb_endpoint_get_string(&endpoint, buf, 255));
